...
class NICE(object) :
    def __init__(self, args):
        ...
        # 新增，用于做FID-Epoch实验
        self.dataset_phase = args.dataset_phase
    ...
    def train(self):
        ...
        self.start_iter = 1
        if self.resume:
            params = torch.load(os.path.join(self.result_dir, os.path.basename(self.dataset), 'model', os.path.basename(self.dataset) + '_params_latest.pt'))
        ...
        for step in range(self.start_iter, self.iteration + 1):
            ...
            # 每10000个iter保存一次模型，路径为 ./results/sar2opt/model/sar2opt_params_10000.pt
            if step % self.save_freq == 0:
            # if step % 2 == 0:
                save_root_dir = os.path.join(self.result_dir, os.path.basename(self.dataset), 'model')
                self.save(save_root_dir, step)
            ...
            # if step % 5 == 0:
                ...
                self.save_path(os.path.join(save_root_dir, os.path.basename(self.dataset) + "_params_latest.pt"), step)
    ...
    def save(self, dir, step):
        ...
        # 模型保存目录为 ./dir/sar2opt_params_10000.pt
        torch.save(params, os.path.join(dir, 'params_%07d.pt' % step))


    def save_path(self, save_dir, step):
        params = {}
        params['gen2B'] = self.gen2B.state_dict()
        params['gen2A'] = self.gen2A.state_dict()
        params['disA'] = self.disA.state_dict()
    ...
    def load(self):
        ...
        # load_path = os.path.join(self.result_dir, os.path.basename(self.dataset), 'model', checkpoint)

        load_path = os.path.join(self.result_dir, os.path.basename(self.dataset), 'model', os.path.basename(self.dataset) + '_params_latest.pt')
        # load_path = os.path.join(self.result_dir, os.path.basename(self.dataset), 'model', 'params_0200000.pt')
    ...
    def test(self):
        ...
        for n, (real_A, real_A_path) in enumerate(self.testA_loader):
            ...
            print(real_A_path[0])
            cv2.imwrite(os.path.join(self.result_dir, os.path.basename(self.dataset), 'fakeB', real_A_path[0].split('/')[-1]), A2B * 255.0)
        ...
        for n, (real_B, real_B_path) in enumerate(self.testB_loader):
            ...
            print(real_B_path[0])
            cv2.imwrite(os.path.join(self.result_dir, os.path.basename(self.dataset), 'fakeA', real_B_path[0].split('/')[-1]), B2A * 255.0)
