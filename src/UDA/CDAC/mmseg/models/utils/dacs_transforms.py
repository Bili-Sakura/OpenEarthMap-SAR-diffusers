...
def get_mean_std(img_metas, dev):
    ...
    mean = torch.stack(mean).view(-1, 3, 1, 1)      # modified (-1, 3, 1, 1)=>(-1, 1, 1, 1) for SAR | (-1, 4, 1, 1)  for SAR+RGB
    std = [
        torch.as_tensor(img_metas[i]['img_norm_cfg']['std'], device=dev)
        for i in range(len(img_metas))
    ]
    std = torch.stack(std).view(-1, 3, 1, 1)       # modified (-1, 3, 1, 1)=>(-1, 1, 1, 1) for SAR | (-1, 4, 1, 1)  for SAR+RGB
    return mean, std
