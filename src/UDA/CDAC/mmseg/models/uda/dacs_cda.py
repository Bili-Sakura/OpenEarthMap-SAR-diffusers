...
class DACS_CDA(UDADecorator):
    ...
    def compute_attn_loss(self, src_attns, mix_attns, tgt_attns, mix_masks, valid_masking=False):
        ...
        for index in range(len(mix_attns)): # 1 src, 0 tgt
            ...
            losses.append(loss)
        attn_loss, attn_log = self._parse_losses({'loss_attn': sum(losses)/len(losses)})
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img, target_img_metas):
        ...
        if self.local_iter % self.debug_img_interval == 0:
            ...
            for j in range(batch_size):
                ...
                # subplotimg(axs[0][1], gt_semantic_seg[j], 'Source Seg GT', cmap='oem')
                # subplotimg(axs[1][1], pseudo_label[j], 'Target Seg (Pseudo) GT', cmap='oem')
                # # subplotimg(axs[0][2], vis_mixed_img[j], 'Mixed Image')
                # # subplotimg(axs[1][2], mix_masks[j][0], 'Domain Mask', cmap='gray')
                # # subplotimg(axs[0][3], pred_u_s[j], "Seg Pred", cmap="oem")
                # # subplotimg(axs[1][3], mixed_lbl[j], 'Seg Targ', cmap='oem')
                # subplotimg(axs[0][2], vis_aug_img[j], 'Mixed Image')
                # subplotimg(axs[1][2], mix_masks[j][0], 'Domain Mask', cmap='gray')
                # subplotimg(axs[0][3], mixed_lbl[j], 'Seg Targ', cmap='oem')
                # subplotimg(axs[1][3], pseudo_weight_tgt[j], 'Pseudo W.', vmin=0, vmax=1)
            ...
            out_dir_4_branch = os.path.join(self.train_cfg['work_dir'], '4_branch_debug')
            ...
            for j in range(batch_size):
                ...
                for ax in axs.flat:
                    ax.axis('off')
                plt.savefig(os.path.join(out_dir_4_branch, f'{(self.local_iter + 1):06d}_{j}.png'))
                plt.close()
